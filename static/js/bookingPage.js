...
function updateDatesAndDays() {
// Get the current date
let currentDate = new Date();

// Array of day names
let daysOfWeek = ["Sun", "Mon", "Tue", "Wed", "Thur", "Fri", "Sa"];

// Iterate through the date input elements (d1, d2, d3, etc.)
for (let i = 1; i <= 7; i++) {
    // Calculate the date for each element
    let dateInputElement = document.querySelector(`#d${i}`);
    let targetDate = new Date(currentDate);
    targetDate.setDate(currentDate.getDate() + i - 1);
    
    // Get the day of the week and date
    let dayName = daysOfWeek[targetDate.getDay()];
    let dayDate = targetDate.getDate();
    
    // Find the associated label for each date input element
    let labelElement = document.querySelector(`label[for="d${i}"]`);
    
    if (labelElement) {
        // Select the day and date divs within the label
        let dayElement = labelElement.querySelector(".day");
        let dateTextElement = labelElement.querySelector(".date");
        
        // Update the day and date text
        if (dayElement) {
            dayElement.textContent = dayName;
        }
        if (dateTextElement) {
            dateTextElement.textContent = dayDate;
        }
    }
}
}
...
// Call the function to update the dates and days when the page loads
